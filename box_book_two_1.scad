...
bw3 = 142+8+5+8; // left to right (to magnet holder)
bw4 = 74; // across panel
    ...
    difference() {
        union() {
            ...
            translate([0,0,wt]) {
                ...
                translate([bw4+20,bw1-shi,0]) rotate([0,0,180]) support_pillar(support_pillar_height);
            }
        }
        panel_cutouts();
...
module lid() {
    difference() {
        union() {
            ...
            translate([bw4+20,bw1-shi,0]) rotate([0,0,180]) lid_mount();
        ...
        translate([bw4+20,bw1-shi,0]) rotate([0,0,180]) screwhole();
        i=12;
...
module panel_cutouts() {
    translate([40+20,bw1,bwh/2+10]) rotate([90,0,0]) cylinder(wt*5, d=pd, center=true);
    translate([40+20,bw1,bwh/2-10+2]) rotate([90,0,0]) cylinder(wt*5, d=pd, center=true);
    translate([20+20,bw1,bwh/2+1]) rotate([90,0,0]) cylinder(wt*5, d=sd, center=true);
}
